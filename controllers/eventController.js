...
const getEvents = asyncHandler(async (req, res) => {
  const sqlQuery = "SELECT * from RadioEvents Order by Created_At DESC";
...
const addEvent = asyncHandler(async (req, res) => {
  ...
  const { name, title, description, date, image } = req.body;

  if (!name || !title || !description || !date || !image) {
    res.status(400);
    throw new Error("All are mandatory fields");
  }

  const sqlQuery = `INSERT INTO RadioEvents(Event_Name,Event_Title,Event_Description,Event_Date,Created_At, Event_Image)  VALUES ('${name}', '${title}','${description}','${date}', now(), '${image}' )`;
...
const editEvent = asyncHandler(async (req, res) => {
  ...
  const { id, name, title, description, date, image } = await req.body;
  ...
  const sqlQuery = `UPDATE RadioEvents SET Event_Name = '${name}', Event_Title ='${title}', Event_Description = '${description}' , Event_Date = '${date}', Event_Image = '${image}' WHERE Event_Id = '${id}'`;
  dbConfig.query(sqlQuery, (err, result) => {
    if (err) return res.status(400).json({ Message: "Error in api" + err });
